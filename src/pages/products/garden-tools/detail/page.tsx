...
export default function GardenToolDetailPage() {
  ...
  useEffect(() => {
    if (!isMainHovered && !mainSwipe.isSwiping) {
      mainIntervalRef.current = setInterval(() => { setMainBgAnim(true); setCurrentMainImage((prev) => (prev + 1) % productData.mainImages.length); setMainAnimKey((k) => k + 1); }, 3000);
    }
  ...
  useEffect(() => {
    ...
    if (!isAccessoryHovered && !accSwipe.isSwiping) {
      accessoryIntervalRef.current = setInterval(() => { setAccBgAnim(true); setAccAnimDir('right'); setCurrentAccessory((prev) => (prev + 1) % productData.accessories.length); setAccAnimKey((k) => k + 1); }, 3000);
    }
