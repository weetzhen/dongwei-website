...
import { sharpeningDetailData, sharpeningCategoryAccessories } from '@/mocks/sharpening-detail';
import { sharpeningSpecsData } from '@/mocks/sharpening-specs';

export default function SharpeningDetailPage() {
  const { t, i18n } = useTranslation();
  const { category, productId } = useParams<{ category: string; productId: string }>();

  const mockEntry = productId ? sharpeningDetailData[productId] : undefined;

  const getSharpeningSpecs = () => {
    if (category === 'drill-repair') {
      return [
        { label: t('spec_power'), value: '150W' }, { label: t('spec_grinding_wheel'), value: '150mm' },
        { label: t('spec_speed'), value: '3000 RPM' }, { label: t('spec_weight'), value: '3.5 kg' },
        { label: t('spec_voltage'), value: 'AC 220V' }, { label: t('spec_chuck_size'), value: '3–13mm' },
        { label: t('spec_max_torque'), value: '2.0 N·m' }, { label: t('spec_noise_level'), value: '≤75 dB' },
        { label: t('spec_dimensions'), value: '320×200×260 mm' }, { label: t('spec_color'), value: t('spec_val_color_silver_black') },
        { label: t('spec_certification'), value: 'CE / EMC' }, { label: t('spec_warranty'), value: t('spec_val_warranty_12') },
      ];
    }
    if (category === 'engraving') {
      return [
        { label: t('spec_power'), value: '65W' }, { label: t('spec_speed'), value: '5000–30000 RPM' },
        { label: t('spec_chuck_size'), value: '3.2mm' }, { label: t('spec_weight'), value: '0.6 kg' },
        { label: t('spec_voltage'), value: 'AC 220V' }, { label: t('spec_temperature'), value: '< 50°C' },
        { label: t('spec_max_torque'), value: '0.2 N·m' }, { label: t('spec_noise_level'), value: '≤70 dB' },
        { label: t('spec_dimensions'), value: '200×35×35 mm' }, { label: t('spec_color'), value: t('spec_val_color_silver_black') },
        { label: t('spec_certification'), value: 'CE / RoHS' }, { label: t('spec_warranty'), value: t('spec_val_warranty_12') },
      ];
    }
    return [
      { label: t('spec_power'), value: '200W' }, { label: t('spec_grinding_wheel'), value: '150mm' },
      { label: t('spec_speed'), value: '1450 RPM' }, { label: t('spec_weight'), value: '5.2 kg' },
      { label: t('spec_voltage'), value: 'AC 220V' }, { label: t('spec_disc_size'), value: '150×20×32mm' },
      { label: t('spec_max_torque'), value: '2.5 N·m' }, { label: t('spec_noise_level'), value: '≤75 dB' },
      { label: t('spec_dimensions'), value: '400×200×300 mm' }, { label: t('spec_color'), value: t('spec_val_color_silver_black') },
      { label: t('spec_certification'), value: 'CE / EMC / RoHS' }, { label: t('spec_warranty'), value: t('spec_val_warranty_24') },
    ];
  };
  ...
  const getProductInfo = (): { name: string; model: string } => {
    ...
    if (category === 'workbench') {
      const num = productId.replace('wb-', '').padStart(3, '0');
      return { name: t(`products.sharpening.workbench.${num}.name`), model: t(`products.sharpening.workbench.${num}.model`) };
    }
    if (category === 'drill-repair') {
      const num = productId.replace('dr-', '').padStart(3, '0');
      return { name: t(`products.sharpening.drillRepair.${num}.name`), model: t(`products.sharpening.drillRepair.${num}.model`) };
    }
    if (category === 'engraving') {
      const num = productId.replace('eg-', '').padStart(3, '0');
      return { name: t(`products.sharpening.engraving.${num}.name`), model: t(`products.sharpening.engraving.${num}.model`) };
    }
    return { name: productId.toUpperCase(), model: productId.toUpperCase() };
  };

  const getCategoryName = () => {
    if (category === 'workbench') return t('cat_sharpening_workbench');
    if (category === 'drill-repair') return t('cat_sharpening_drill_repair');
    if (category === 'engraving') return t('cat_sharpening_engraving');
    return t('cat_sharpening');
  };

  const productInfo = getProductInfo();

  const pid = productId || 'wb-001';
  ...
  const getCategoryAccessoryKey = (baseKey: string): string => {
    const cat = category === 'drill-repair' ? 'dr' : category === 'engraving' ? 'eg' : 'wb';
    return baseKey.replace(/^accessory_(\d+)_(name|desc)$/, `sharpening_${cat}_acc_$1_$2`);
  };

  const rawSpecItems = sharpeningSpecsData[pid];
  ...
  const getFallbackPrefix = (id: string) => {
    if (id.startsWith('dr')) return 'sharpening_drill';
    if (id.startsWith('eg')) return 'sharpening_engraving';
    return 'sharpening_workbench';
  };
  ...
  const productData = {
    id: pid, name: productInfo.name, model: productInfo.model, category: category || 'workbench',
    mainImages: mockEntry?.mainImages ?? [
      `https://readdy.ai/api/search-image?query=professional%20electric%20sharpening%20tool%20silver%20black%20metallic%20on%20clean%20white%20background%20industrial%20product%20photography%20studio%20lighting%20high%20resolution%20front%20view&width=600&height=600&seq=sp-det-${pid}-1&orientation=squarish`,
      `https://readdy.ai/api/search-image?query=professional%20electric%20sharpening%20tool%20silver%20black%20metallic%20on%20clean%20white%20background%20industrial%20product%20photography%20studio%20lighting%20high%20resolution%20side%20view&width=600&height=600&seq=sp-det-${pid}-2&orientation=squarish`,
      `https://readdy.ai/api/search-image?query=professional%20electric%20sharpening%20tool%20silver%20black%20metallic%20on%20clean%20white%20background%20industrial%20product%20photography%20studio%20lighting%20high%20resolution%20top%20view&width=600&height=600&seq=sp-det-${pid}-3&orientation=squarish`,
      `https://readdy.ai/api/search-image?query=professional%20electric%20sharpening%20tool%20silver%20black%20metallic%20on%20clean%20white%20background%20industrial%20product%20photography%20studio%20lighting%20high%20resolution%20back%20view&width=600&height=600&seq=sp-det-${pid}-4&orientation=squarish`,
      `https://readdy.ai/api/search-image?query=professional%20electric%20sharpening%20tool%20silver%20black%20metallic%20on%20clean%20white%20background%20industrial%20product%20photography%20studio%20lighting%20high%20resolution%20detail%20view&width=600&height=600&seq=sp-det-${pid}-5&orientation=squarish`,
      `https://readdy.ai/api/search-image?query=professional%20electric%20sharpening%20tool%20silver%20black%20metallic%20on%20clean%20white%20background%20industrial%20product%20photography%20studio%20lighting%20high%20resolution%20angle%20view&width=600&height=600&seq=sp-det-${pid}-6&orientation=squarish`,
      `https://readdy.ai/api/search-image?query=professional%20electric%20sharpening%20tool%20silver%20black%20metallic%20on%20clean%20white%20background%20industrial%20product%20photography%20studio%20lighting%20high%20resolution%20close%20up%20view&width=600&height=600&seq=sp-det-${pid}-7&orientation=squarish`,
      `https://readdy.ai/api/search-image?query=professional%20electric%20sharpening%20tool%20silver%20black%20metallic%20on%20clean%20white%20background%20industrial%20product%20photography%20studio%20lighting%20high%20resolution%20perspective%20view&width=600&height=600&seq=sp-det-${pid}-8&orientation=squarish`,
    ],
    features: productFeatures, description: description, specs: resolvedSpecs,
    accessories: (mockEntry?.accessories ?? sharpeningCategoryAccessories).map((a, idx) => {
  ...
  useEffect(() => { window.scrollTo({ top: 0, behavior: 'instant' }); setMainBgAnim(true); setAccBgAnim(true); }, [category, productId]);

  useEffect(() => {
    if (!isMainHovered && !mainSwipe.isSwiping) { mainIntervalRef.current = setInterval(() => { setMainBgAnim(true); setCurrentMainImage((prev) => (prev + 1) % productData.mainImages.length); setMainAnimKey((k) => k + 1); }, 3000); }
  ...
  useEffect(() => {
    ...
    if (!isAccessoryHovered && !accSwipe.isSwiping) { accessoryIntervalRef.current = setInterval(() => { setAccBgAnim(true); setAccAnimDir('right'); setCurrentAccessory((prev) => (prev + 1) % productData.accessories.length); setAccAnimKey((k) => k + 1); }, 3000); }
  ...
  const handleMainPrev = () => { setMainBgAnim(true); setCurrentMainImage((prev) => (prev - 1 + productData.mainImages.length) % productData.mainImages.length); setMainAnimKey((k) => k + 1); };
  const handleMainNext = () => { setMainBgAnim(true); setCurrentMainImage((prev) => (prev + 1) % productData.mainImages.length); setMainAnimKey((k) => k + 1); };
  const handleAccessoryPrev = () => { if (productData.accessories.length <= 1) return; setAccBgAnim(true); setAccAnimDir('left'); setCurrentAccessory((prev) => (prev - 1 + productData.accessories.length) % productData.accessories.length); setAccAnimKey((k) => k + 1); };
  const handleAccessoryNext = () => { if (productData.accessories.length <= 1) return; setAccBgAnim(true); setAccAnimDir('right'); setCurrentAccessory((prev) => (prev + 1) % productData.accessories.length); setAccAnimKey((k) => k + 1); };
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-4 md:py-6">
        <div className="flex items-center gap-2 text-xs md:text-sm text-gray-500">
          ...
          <span>&gt;</span>
          <Link to="/products/sharpening" className="hover:text-[#f6444e] transition-colors cursor-pointer">{t('cat_sharpening')}</Link>
          <span>&gt;</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12">
          ...
          <div className="space-y-4 md:space-y-6">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-black text-gray-900 mb-4 md:mb-4">{productData.name}</h1>
            </div>
          ...
          <div className="flex flex-col gap-8">
            <div>
              ...
              <div className="border border-[#f6444e] rounded-lg">
                <div className="flex items-center justify-center px-4 py-1.5 bg-[#f6444e] rounded-t-lg"><span className="text-sm text-white font-bold tracking-widest uppercase">{productData.model}</span></div>
                <div className="divide-y divide-gray-100">
                  {productData.specs.map((spec, idx) => (<div key={idx} className={`flex items-start px-4 py-2 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} ${idx === productData.specs.length - 1 ? 'rounded-b-lg' : ''}`}><span className="w-[45%] flex-shrink-0 text-sm text-gray-500 font-medium leading-relaxed">{spec.label}</span><span className="flex-1 min-w-0 text-sm text-gray-900 font-semibold break-words leading-relaxed">{spec.value}</span></div>))}
                </div>
              </div>
            </div>
            <div>
